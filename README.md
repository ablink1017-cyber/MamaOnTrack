...
function BreastfeedingTimer({ onSave }) {
 ...
 Tap a side to start. Tap again to pause. Switch sides anytime.
 ...
 const color = side === "Left" ? th.accent : th.purple;
 return (
 <button
 key={side}
 onClick={() => toggleSide(side)}
 style={{
 flex:1, padding:"16px 12px", borderRadius:6,
 background: active ? color : hasTime ? `${color}18` : "#fff",
 border:`2px solid ${active ? color : hasTime ? `${color}66` : th.border}`,
 cursor:"pointer", transition:"all 0.2s",
 boxShadow: active ? `0 4px 20px ${color}55` : "none",
 }}>
 <div style={{ fontSize:28, marginBottom:4 }}>{side === "Left" ? "" : ""}</div>
 <div style={{ fontWeight:500, fontSize:14, color: active ? "#fff" : color }}>
 {side} Side
 </div>
 <div style={{
 fontSize:28, fontWeight:500, fontFamily:"monospace",
 color: active ? "#fff" : hasTime ? color : th.muted,
 marginTop:6, letterSpacing:1,
 }}>
 {fmt(secs)}
 </div>
 <div style={{ fontSize:11, color: active ? "rgba(255,255,255,0.8)" : th.muted, marginTop:4 }}>
 {active ? " Tap to pause" : secs > 0 ? " Tap to resume" : " Tap to start"}
 </div>
 </button>
 );
 })}
 ...
 <div style={{ fontWeight:500, fontSize:14, color:th.text }}>{fmt(totalSecs)}</div>
 </div>
 {/* Side split bar */}
 <div style={{ background:th.border, borderRadius:999, height:10, overflow:"hidden", display:"flex" }}>
 {leftSecs > 0 && (
 <div style={{ width:`${(leftSecs/totalSecs)*100}%`, background:`linear-gradient(90deg,${th.accent},#ff6b9d)`, transition:"width 0.3s" }} />
 )}
 {rightSecs > 0 && (
 <div style={{ flex:1, background:`linear-gradient(90deg,${th.purple},#a030e0)`, transition:"flex 0.3s" }} />
 )}
 </div>
 <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}>
 <div style={{ fontSize:11, color:th.accent, fontWeight:500 }}> Left {fmt(leftSecs)}</div>
 <div style={{ fontSize:11, color:th.purple, fontWeight:500 }}>Right {fmt(rightSecs)} </div>
 </div>
 </div>
 )}
 ...
 <button
 onClick={handleSave}
 style={{ flex:1, padding:"12px", borderRadius:999, background:`linear-gradient(135deg,${th.accent},${th.purple})`, color:"#fff", border:"none", cursor:"pointer", fontWeight:500, fontSize:14, boxShadow:`0 3px 12px ${th.accent}44` }}>
 Save Session ({totalMins} min)
 </button>
 )}
 {totalSecs > 0 && (
 <button
 onClick={reset}
 style={{ padding:"12px 18px", borderRadius:999, background:"transparent", color:th.muted, border:`1px solid ${th.border}`, cursor:"pointer", fontSize:13, fontWeight:500 }}>
 Reset
 </button>
 )}
 </div>
...
function AISleepPlanner({ kid, sleepLog }) {
 ...
 {kid.name}&apos;s Sleep Program
 </div>
 <div style={{fontSize:10,color:th.muted,letterSpacing:"0.12em",textTransform:"uppercase",fontFamily:"'Inter',sans-serif"}}>
 {prog.label} &middot; Premium Feature
 </div>
 </div>

 <div style={{display:"flex",gap:4,marginBottom:16,background:"#faf8f5",border:"1px solid " + th.border,borderRadius:6,padding:4}}>
 {tabs.map(t => (
 <button key={t.id} onClick={() => setActiveTab(t.id)} style={{flex:1,padding:"8px 4px",borderRadius:4,background:activeTab===t.id?th.accent:"transparent",color:activeTab===t.id?"#fff":th.muted,border:"none",cursor:"pointer",fontSize:10,fontWeight:500,letterSpacing:"0.04em",textTransform:"uppercase",fontFamily:"'Inter',sans-serif"}}>
 {t.label}
 </button>
 ))}
 </div>
 ...
 <p style={{margin:"0 0 14px",fontSize:13,lineHeight:1.9,color:th.text,fontFamily:"'Inter',sans-serif"}}>{prog.about}</p>
 <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
 {[{l:"Total Sleep",v:prog.total,c:th.purple},{l:"Night Sleep",v:prog.night,c:th.blue},{l:"Naps",v:prog.naps,c:th.accent},{l:"Nap Length",v:prog.napDur,c:th.green}].map((g,i) => (
 <div key={i} style={{background:g.c+"10",border:"1px solid "+g.c+"28",borderRadius:6,padding:"8px 14px"}}>
 <div style={{fontSize:10,color:th.muted,fontWeight:500,letterSpacing:"0.08em",textTransform:"uppercase",fontFamily:"'Inter',sans-serif"}}>{g.l}</div>
 <div style={{fontSize:16,fontWeight:300,color:g.c,fontFamily:"'Inter',sans-serif"}}>{g.v}</div>
 </div>
 ))}
 </div>
 </div>
 <div style={{background:"#faf8f5",border:"1px solid "+th.border,borderRadius:6,padding:16,marginBottom:12}}>
 <div style={{fontSize:10,color:th.muted,letterSpacing:"0.12em",textTransform:"uppercase",fontFamily:"'Inter',sans-serif",marginBottom:8}}>A Note From Your Nurse</div>
 <p style={{margin:0,fontSize:14,lineHeight:1.8,color:th.text,fontFamily:"'Cormorant Garamond',serif",fontStyle:"italic"}}>{prog.nurseNote}</p>
 </div>
 <div style={{fontSize:10,color:th.muted,letterSpacing:"0.14em",textTransform:"uppercase",fontFamily:"'Inter',sans-serif",marginBottom:10}}>Week by Week Plan</div>
 {prog.weeklyPlan.map((w,i) => (
 <div key={i} onClick={() => setWeekOpen(weekOpen===i?null:i)} style={{background:"#ffffff",border:"1px solid "+th.border,borderRadius:6,padding:14,marginBottom:8,cursor:"pointer"}}>
 <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
 <div>
 <div style={{fontSize:10,color:th.muted,letterSpacing:"0.08em",textTransform:"uppercase",fontFamily:"'Inter',sans-serif"}}>{w.week}</div>
 <div style={{fontSize:13,fontWeight:500,color:th.text,fontFamily:"'Inter',sans-serif",marginTop:2}}>{w.focus}</div>
 </div>
 <div style={{fontSize:11,color:th.muted}}>{weekOpen===i ? "" : ""}</div>
 </div>
 {weekOpen===i && <p style={{margin:"10px 0 0",fontSize:13,lineHeight:1.8,color:th.muted,fontFamily:"'Inter',sans-serif"}}>{w.desc}</p>}
 </div>
 ))}
 <div style={{fontSize:10,color:th.muted,marginTop:8,fontFamily:"'Inter',sans-serif",textAlign:"center"}}>Source: AAP/AASM Pediatric Sleep Guidelines &middot; Always consult your pediatrician</div>
 </div>
 )}
 ...
 {prog.schedule.map((item,i) => {
 const isNap=item.label.toLowerCase().includes("nap");
 const isBed=item.label.toLowerCase().includes("bedtime");
 const isWake=item.label.toLowerCase().includes("wake");
 const col=isNap?th.purple:isBed?th.blue:isWake?th.yellow:th.green;
 return (
 <div key={i} style={{display:"flex",alignItems:"center",gap:14,padding:"10px 0",borderBottom:i<prog.schedule.length-1?"1px solid "+th.border:"none"}}>
 <div style={{width:70,fontSize:12,color:th.muted,fontFamily:"'Inter',sans-serif",flexShrink:0}}>{item.time}</div>
 <div style={{width:3,height:32,borderRadius:999,background:col,flexShrink:0}} />
 <div style={{fontSize:13,color:th.text,fontFamily:"'Inter',sans-serif"}}>{item.label}</div>
 </div>
 );
 })}
 </div>
 <div style={{background:"#faf8f5",border:"1px solid "+th.border,borderRadius:6,padding:14}}>
 <p style={{margin:0,fontSize:12,lineHeight:1.8,color:th.muted,fontFamily:"'Inter',sans-serif"}}>This is a sample schedule, not a rigid prescription. Adjust by 30-60 minutes to fit your family. Watch your baby&#39;s sleepy cues over the clock. Every baby is different.</p>
 </div>
 </div>
 )}
 ...
 <div style={{fontWeight:400,fontSize:22,fontFamily:"'Cormorant Garamond',serif",fontStyle:"italic",marginBottom:10}}>Not yet &mdash; and that is okay.</div>
 <p style={{fontSize:13,lineHeight:1.9,color:th.text,fontFamily:"'Inter',sans-serif",margin:"0 0 12px"}}>Sleep training is not developmentally appropriate before 4 months. Newborns cannot self-soothe. Any crying at this age requires a response.</p>
 <p style={{fontSize:13,lineHeight:1.9,color:th.muted,fontFamily:"'Inter',sans-serif",margin:0}}>Focus on establishing a simple bedtime routine now. The groundwork you lay in these weeks makes sleep training much easier when the time comes.</p>
 </div>
 ) : (
 <div>
 <div style={{fontSize:10,color:th.muted,letterSpacing:"0.12em",textTransform:"uppercase",fontFamily:"'Inter',sans-serif",marginBottom:12}}>Choose Your Method</div>
 {(prog.methods||[]).map(m => {
 const met=SLEEP_METHODS[m];
 const sel=method===m;
 return (
 <div key={m} onClick={() => setMethod(sel?null:m)} style={{background:sel?met.color+"12":"#ffffff",border:"1px solid "+(sel?met.color:th.border),borderRadius:6,padding:16,marginBottom:10,cursor:"pointer"}}>
 <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:sel?10:0}}>
 <div>
 <div style={{fontSize:13,fontWeight:500,color:sel?met.color:th.text,fontFamily:"'Inter',sans-serif"}}>{met.name}</div>
 <div style={{fontSize:11,color:th.muted,marginTop:2,fontFamily:"'Inter',sans-serif"}}>Best for: {met.best}</div>
 </div>
 <div style={{width:18,height:18,borderRadius:"50%",border:"1px solid "+(sel?met.color:th.border),background:sel?met.color:"transparent",display:"flex",alignItems:"center",justifyContent:"center"}}>
 {sel && <div style={{width:8,height:8,borderRadius:"50%",background:"#fff"}} />}
 </div>
 </div>
 {sel && (
 <div>
 <p style={{margin:"0 0 10px",fontSize:13,lineHeight:1.8,color:th.text,fontFamily:"'Inter',sans-serif"}}>{met.desc}</p>
 <div style={{fontSize:10,color:th.muted,letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"'Inter',sans-serif",marginBottom:8}}>How to do it</div>
 {met.steps.map((step,si) => (
 <div key={si} style={{display:"flex",gap:10,marginBottom:6}}>
 <div style={{width:18,height:18,borderRadius:3,background:met.color,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:500,flexShrink:0}}>{si+1}</div>
 <div style={{fontSize:12,color:th.muted,lineHeight:1.7,fontFamily:"'Inter',sans-serif"}}>{step}</div>
 </div>
 ))}
 <div style={{marginTop:10,padding:"8px 12px",background:met.color+"10",borderRadius:4}}>
 <div style={{fontSize:10,color:th.muted,letterSpacing:"0.08em",textTransform:"uppercase",fontFamily:"'Inter',sans-serif",marginBottom:4}}>Expected timeline</div>
 <div style={{fontSize:12,color:met.color,fontFamily:"'Inter',sans-serif"}}>{met.timeline}</div>
 </div>
 <div style={{marginTop:8,fontSize:10,color:th.muted,fontFamily:"'Inter',sans-serif",fontStyle:"italic"}}>{met.source}</div>
 </div>
 )}
 </div>
 );
 })}
 {!method && <div style={{background:"#faf8f5",border:"1px solid "+th.border,borderRadius:6,padding:14,fontSize:12,color:th.muted,fontFamily:"'Inter',sans-serif",lineHeight:1.7}}>Tap any method above to learn more. Select one then use the AI Planner tab for a personalized plan.</div>}
 </div>
 )}
 </div>
 )}
 ...
 <p style={{margin:"0 0 14px",fontSize:12,color:th.muted,lineHeight:1.7,fontFamily:"'Inter',sans-serif"}}>Powered by Claude AI &middot; AAP/AASM guidelines{method?" + "+SLEEP_METHODS[method].name:""}</p>
 {!hasEnoughData && <div style={{background:th.yellow+"15",border:"1px solid "+th.yellow+"33",borderRadius:4,padding:"8px 12px",marginBottom:14,fontSize:12,color:th.text,fontFamily:"'Inter',sans-serif"}}>Log at least 3 sleep sessions for a personalized plan.</div>}
 <button onClick={generatePlan} disabled={loading} style={{width:"100%",padding:"11px",borderRadius:4,background:loading?"#e0dbd5":th.accent,color:"#fff",border:"none",cursor:loading?"not-allowed":"pointer",fontSize:10,fontWeight:500,letterSpacing:"0.08em",textTransform:"uppercase",fontFamily:"'Inter',sans-serif"}}>
 {loading?"Generating...":plan?"Regenerate Plan":"Generate Sleep Plan"}
 </button>
 {error && <div style={{marginTop:10,fontSize:12,color:th.coral}}>{error}</div>}
 </div>
 {plan && (
 <div style={{background:"#ffffff",border:"1px solid "+th.border,borderRadius:6,padding:20}}>
 <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
 <div style={{fontWeight:400,fontSize:24,fontFamily:"'Cormorant Garamond',serif",fontStyle:"italic"}}>{kid.name}&apos;s Plan</div>
 <button onClick={() => setExpanded(!expanded)} style={{background:"none",border:"none",color:th.muted,cursor:"pointer",fontSize:11,fontFamily:"'Inter',sans-serif"}}>{expanded?"Hide":"Details"}</button>
 </div>
 <div style={{background:th.purple+"08",border:"1px solid "+th.purple+"22",borderRadius:6,padding:"10px 14px",marginBottom:14,fontSize:13,lineHeight:1.8,color:th.text,fontFamily:"'Inter',sans-serif"}}>{plan.summary}</div>
 <div style={{fontSize:10,color:th.muted,letterSpacing:"0.12em",textTransform:"uppercase",fontFamily:"'Inter',sans-serif",marginBottom:10}}>Recommended Schedule</div>
 <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:14}}>
 {[{l:"Wake Up",t:plan.wakeTime,c:th.yellow},...(plan.naps||[]).map(n=>({l:n.label,t:n.time+" - "+n.duration,c:th.purple})),{l:"Bedtime",t:plan.bedtime,c:th.blue},{l:"Total Sleep",t:plan.totalSleep,c:th.green}].map((item,i) => (
 <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:item.c+"0a",border:"1px solid "+item.c+"22",borderRadius:6}}>
 <div style={{width:3,height:32,borderRadius:999,background:item.c,flexShrink:0}} />
 <div>
 <div style={{fontSize:10,color:th.muted,letterSpacing:"0.06em",textTransform:"uppercase",fontFamily:"'Inter',sans-serif"}}>{item.l}</div>
 <div style={{fontSize:13,color:th.text,fontFamily:"'Inter',sans-serif",marginTop:2}}>{item.t}</div>
 </div>
 </div>
 ))}
 </div>
 {expanded && plan.tips && plan.tips.map((tip,i) => (
 <div key={i} style={{display:"flex",gap:10,padding:"8px 0",borderBottom:i<plan.tips.length-1?"1px solid "+th.border:"none"}}>
 <div style={{width:3,borderRadius:999,background:th.accent,flexShrink:0}} />
 <div style={{fontSize:13,lineHeight:1.7,color:th.text,fontFamily:"'Inter',sans-serif"}}>{tip}</div>
 </div>
 ))}
 </div>
 )}
...
function FormField({ label, field, type="text", options, form, setForm }) {
 ...
 {options
 ? <select style={inputStyle} value={form[field]||""} onChange={e=>setForm(prev=>({...prev,[field]:e.target.value}))}><option value="">Select...</option>{options.map(o=><option key={o}>{o}</option>)}</select>
 : <input style={inputStyle} type={type} value={form[field]||""} onChange={e=>setForm(prev=>({...prev,[field]:e.target.value}))} />}
 </div>
 );
}
...
function EntryModal({ modal, form, setForm, setModal, saveEntry }) {
 ...
 <h3 style={{ margin:0, fontSize:18, fontWeight:500 }}>{modal.editing?" Edit Entry":" New Entry"}</h3>
 <button onClick={()=>setModal(null)} style={{ background:"none", border:"none", color:"#8a8480", cursor:"pointer", fontSize:20 }}></button>
 </div>
 {modal.fields.map(f=><FormField key={f.field} {...f} form={form} setForm={setForm} />)}
 <div style={{ display:"flex", gap:10, marginTop:20 }}>
 <button style={{ padding:"10px 22px", borderRadius:4, background:theme.accent, color:"#fff", border:"none", cursor:"pointer", fontSize:11, fontWeight:500, letterSpacing:"0.06em", textTransform:"uppercase", boxShadow:"none", fontFamily:"'Inter',sans-serif" }} onClick={saveEntry}>
 {modal.editing?"Save Changes":"Add Entry"}
 </button>
 <button style={{ padding:"10px 18px", borderRadius:4, background:"transparent", color:theme.accent, border:`1px solid ${theme.accent}`, cursor:"pointer", fontSize:11, fontWeight:500, letterSpacing:"0.06em", textTransform:"uppercase", fontFamily:"'Inter',sans-serif" }} onClick={()=>setModal(null)}>
 Cancel
 </button>
 </div>
 </div>
...
function DeleteModal({ confirmDelete, setConfirmDelete, deleteEntry }) {
 ...
 <h3 style={{ margin:"0 0 8px", fontWeight:500 }}>Delete this entry?</h3>
 <p style={{ color:"#8a8480", margin:"0 0 24px", fontSize:14 }}>This will be permanently removed.</p>
 <div style={{ display:"flex", gap:10, justifyContent:"center" }}>
 <button style={{ padding:"10px 22px", borderRadius:4, background:theme.coral, color:"#fff", border:"none", cursor:"pointer", fontSize:11, fontWeight:500, letterSpacing:"0.06em", textTransform:"uppercase", fontFamily:"'Inter',sans-serif" }} onClick={deleteEntry}>Yes, Delete</button>
 <button style={{ padding:"10px 18px", borderRadius:4, background:"transparent", color:theme.accent, border:`1px solid ${theme.accent}`, cursor:"pointer", fontSize:11, fontWeight:500, letterSpacing:"0.06em", textTransform:"uppercase", fontFamily:"'Inter',sans-serif" }} onClick={()=>setConfirmDelete(null)}>Cancel</button>
 </div>
 </div>
 </div>
 );
...
function EntryCard({ item, dataKey, label, fields, kidScoped=false, children, openEdit, setConfirmDelete, c }) {
 ...
 <button style={c.iconBtn(theme.coral)} onClick={()=>setConfirmDelete({ dataKey, id:item.id, label })}></button>
 </div>
...
function DailyMotivation() {
 ...
 <div style={{ fontSize:22, fontWeight:400, fontFamily:"'Cormorant Garamond',serif", fontStyle:"italic", color:"#1a1a1a", lineHeight:1.5, marginBottom:10 }}>
 &ldquo;{mot.quote}&rdquo;
 </div>
 <div style={{ fontSize:11, color:"#8a8480", letterSpacing:"0.06em", fontFamily:"'Inter',sans-serif" }}>
 &mdash; {mot.author}
 </div>
 </div>
 );
...
export default function App() {
 ...
 {vital.o2 && <div><div style={{ fontSize:22, fontWeight:500, color:theme.teal }}>{vital.o2}%</div><div style={{ fontSize:11, color:theme.muted }}>O2</div></div>}
 {vital.weight && <div><div style={{ fontSize:22, fontWeight:500, color:theme.green }}>{vital.weight} lbs</div><div style={{ fontSize:11, color:theme.muted }}>Weight</div></div>}
 </div>
 </EntryCard>
 );
 ...
 <div>
 <div style={{ fontSize:22, marginBottom:4 }}>{typeIcon(ev.type)}</div>
 <div style={{ fontWeight:500, fontSize:18 }}>{ev.title}</div>
 <div style={{ fontSize:13, color:theme.muted, marginTop:2 }}>{ev.type}</div>
 </div>
 <button onClick={()=>setEventDetail(null)} style={{ background:"none", border:"none", color:theme.muted, cursor:"pointer", fontSize:20, marginTop:-4 }}></button>
 </div>
 <div style={{ display:"flex", gap:8, marginBottom:16, flexWrap:"wrap" }}>
 <span style={{ ...c.badge(col), fontSize:13 }}>{ava} {name}</span>
 {ev.date && <span style={c.badge(theme.blue)}>{formatDate(ev.date)}</span>}
 {ev.time && <span style={c.badge(theme.purple)}>{formatTime(ev.time)}</span>}
 </div>
 {ev.location && <div style={{ fontSize:14, color:theme.text, marginBottom:8 }}> {ev.location}</div>}
 {ev.notes && <div style={{ fontSize:14, color:theme.muted, marginBottom:16 }}> {ev.notes}</div>}
 <div style={{ display:"flex", gap:10 }}>
 <button style={c.btn(theme.blue)} onClick={()=>{ setEventDetail(null); openEditEvent(ev); }}> Edit</button>
 <button style={c.btn(theme.coral)} onClick={()=>{ setConfirmDelete({ dataKey:"events", id:ev.id, label:ev.title }); }}> Delete</button>
 </div>
 </div>
 </div>
 );
 ...
 style={{ height:64, overflow:"hidden", background: isSelected?`${theme.accent}10`: isToday?`${theme.yellow}10`:"#ffffff", border:`1px solid ${isSelected?theme.accent:isToday?theme.yellow:theme.border}`, borderRadius:4, padding:"4px 3px", cursor:"pointer", transition:"all 0.15s", boxSizing:"border-box" }}>
 <div style={{ fontSize:11, fontWeight: isToday||isSelected ? 500 : 400, color: isToday?theme.yellow: isSelected?theme.accent:theme.muted, textAlign:"center", marginBottom:2, lineHeight:1.2, fontFamily:"'Inter',sans-serif" }}>{day}</div>
 <div style={{ display:"flex", flexDirection:"column", gap:1 }}>
 {visibleEvents.map(ev => (
 <div key={ev.id} onClick={e=>{ e.stopPropagation(); setEventDetail(ev); }}
 style={{ background:memberColor(ev.memberId), borderRadius:2, padding:"1px 3px", fontSize:8, color:"#fff", overflow:"hidden", whiteSpace:"nowrap", textOverflow:"ellipsis", cursor:"pointer", lineHeight:1.5, fontFamily:"'Inter',sans-serif" }}>
 {ev.title}
 </div>
 ))}
 {extra > 0 && (
 <div style={{ fontSize:8, color:theme.muted, textAlign:"center", lineHeight:1.4, fontFamily:"'Inter',sans-serif" }}>+{extra} more</div>
 )}
 </div>
 </div>
 );
 ...
 <div style={{ fontSize:12, fontWeight:500, color:theme.muted, textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:8 }}>{formatDate(date)}</div>
 {evs.map(ev => (
 <div key={ev.id} onClick={()=>setEventDetail(ev)}
 style={{ ...c.card, marginBottom:8, cursor:"pointer", borderLeft:`4px solid ${memberColor(ev.memberId)}`, padding:"14px 16px" }}>
 <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
 <div>
 <div style={{ fontWeight:500 }}>{typeIcon(ev.type)} {ev.title}</div>
 ...
 {/* Header */}
 <div style={c.header}>
 <div style={c.logo}>MamaOnTrack</div>
 ...
 <button style={c.mainTab(mainTab==="mom")}      onClick={()=>setMainTab("mom")}>Mama</button>
 <button style={c.mainTab(mainTab==="family")}   onClick={()=>setMainTab("family")}>Family Access</button>
 ...
 <div style={c.sTitle}>Family Color Key</div>
 <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8 }}>
 {allMembers.map(m => (
 <div key={m.id} style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 12px", borderRadius:3, background:`${m.color}10`, border:`1px solid ${m.color}28` }}>
 <div style={{ width:8, height:8, borderRadius:"50%", background:m.color, flexShrink:0 }} />
 <span style={{ fontSize:14, fontWeight:400, fontStyle:"italic", fontFamily:"'Cormorant Garamond',serif", color:m.color, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{m.name}</span>
 </div>
 ))}
 </div>
 </div>
 ...
 <button style={c.smallBtn(calView==="list" ?theme.accent:theme.muted)} onClick={()=>setCalView("list")}> List</button>
 </div>
 <div style={{ display:"flex", gap:6, flexWrap:"wrap", alignItems:"center" }}>
 <span style={{ fontSize:12, color:theme.muted }}>Filter:</span>
 <select style={{ ...c.input, width:"auto", padding:"6px 12px", fontSize:13 }}
 value={filterMember} onChange={e=>setFilterMember(e.target.value)}>
 <option value="all"> Everyone</option>
 {allMembers.map(m=><option key={m.id} value={String(m.id)}>{m.avatar} {m.name}</option>)}
 </select>
 <button style={c.btn()} onClick={()=>openAddEvent()}>Add Event</button>
 </div>
 </div>
 ...
 <span>{k.avatar}</span><span style={{ fontFamily:"'Cormorant Garamond',serif", fontStyle:"italic", fontSize:15 }}>{k.name}</span><span style={{ color:theme.muted, fontSize:11, letterSpacing:"0.04em" }}>Age {k.age}</span>
 </div>
 ))}
 <button style={c.addKidBtn} onClick={()=>setShowAddKid(true)}>+ Add Child</button>
 </div>
 <div style={{ background:"#faf8f5", borderBottom:`1px solid ${theme.border}`, padding:"10px 20px" }}>
 {/* Always visible: Dashboard + Trends + Reports */}
 <div style={{ display:"flex", gap:4, marginBottom:8, justifyContent:"center" }}>
 <button style={c.navBtn(view==="dashboard")} onClick={()=>setView("dashboard")}>Dashboard</button>
 <button style={c.navBtn(view==="kidTrends")} onClick={()=>setView("kidTrends")}>Trends</button>
 </div>
 {/* Grouped collapsible sections */}
 {[
 { label:"Health Records", tabs:[{id:"symptoms",label:"Symptoms"},{id:"medications",label:"Meds"},{id:"visits",label:"Visits"},{id:"vitals",label:"Vitals & Labs"},{id:"allergies",label:"Allergies"}] },
 { label:"Growth & Development", tabs:[{id:"growth",label:"Growth"},{id:"vaccines",label:"Vaccines"},{id:"milestones",label:"Milestones"},...(selectedKid.age < 5?[{id:"firsts",label:"Firsts"}]:[])] },
 { label:"Daily Tracking", tabs:[...((selectedKid.age*12 < 36)?[{id:"infantLog",label:"Infant Log"}]:[]),{id:"kidSleepTab",label:"Sleep"}] },
 { label:"Resources", tabs:[{id:"tips",label:"Nurse Tips"},{id:"kidReports",label:"Reports"}] },
 ].map(group=>{
 const isActive = group.tabs.some(t=>t.id===view);
 return (
 ...
 {group.tabs.map(t=>(
 <button key={t.id} style={c.navBtn(view===t.id)} onClick={()=>setView(t.id)}>{t.label}</button>
 ))}
 </div>
 </details>
 );
 ...
 <div style={c.stat(theme.green)} onClick={()=>setView("medications")}><div style={{ ...c.statNum, color:theme.green }}>{kidMeds.filter(m=>m.active).length}</div><div style={c.statLabel}>Active Meds</div></div>
 <div style={c.stat(theme.accent)} onClick={()=>setView("visits")}><div style={{ ...c.statNum, color:theme.accent }}>{kidVisits.length}</div><div style={c.statLabel}>Visits</div></div>
 <div style={c.stat(theme.purple)} onClick={()=>setView("vaccines")}><div style={{ ...c.statNum, color:theme.purple }}>{kidVaccineList.length}</div><div style={c.statLabel}>Vaccines</div></div>
 <div style={c.stat(theme.teal)} onClick={()=>setView("milestones")}><div style={{ ...c.statNum, color:theme.teal }}>{kidMilestones.length}</div><div style={c.statLabel}>Milestones</div></div>
 <div style={c.stat(theme.orange)} onClick={()=>setView("kidSleepTab")}><div style={{ ...c.statNum, color:theme.orange }}>{kidSleepLog.length}</div><div style={c.statLabel}>Sleep Logs</div></div>
 </div>
 <div style={c.card}>
 <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
 ...
 <div>
 {events.filter(e=>String(e.memberId)===String(selectedKid.id)&&e.date>=today).sort((a,b)=>a.date.localeCompare(b.date)).slice(0,5).map((ev,i,arr) => (
 <div key={ev.id} style={{ display:"flex", gap:12, padding:"10px 0", borderBottom:i<arr.length-1?`1px solid ${theme.border}`:"none", alignItems:"center" }}>
 <div style={{ width:40, flexShrink:0, textAlign:"center" }}>
 <div style={{ fontSize:18, fontWeight:300, fontFamily:"'Inter',sans-serif", color:theme.accent, lineHeight:1 }}>{ev.date.slice(8)}</div>
 <div style={{ fontSize:9, color:theme.muted, textTransform:"uppercase", letterSpacing:"0.08em", fontFamily:"'Inter',sans-serif" }}>{["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][parseInt(ev.date.slice(5,7))-1]}</div>
 </div>
 ...
 <div style={c.card}>
 <div style={c.sTitle}>Nurse Tip of the Day</div>
 <div style={{ display:"flex", gap:14 }}><span style={{ fontSize:28 }}>{nurseTips[new Date().getDate()%nurseTips.length].icon}</span><p style={{ margin:0, lineHeight:1.7 }}>{nurseTips[new Date().getDate()%nurseTips.length].tip}</p></div>
 </div>
 </>}
 ...
 <div style={{ color:theme.muted, fontSize:13 }}>Log at least 3 symptoms to see patterns.</div>
 ) : (() => {
 const counts = {};
 kidSymptoms.forEach(s => { counts[s.symptom] = (counts[s.symptom]||0)+1; });
 const sorted = Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,6);
 const max = sorted[0][1];
 return sorted.map(([name,count]) => (
 <div key={name} style={{ marginBottom:12 }}>
 <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
 <div style={{ fontSize:13, fontFamily:"'Inter',sans-serif" }}>{name}</div>
 <div style={{ fontSize:12, color:theme.muted }}>{count}x</div>
 </div>
 <div style={{ background:"#f0ebe4", borderRadius:999, height:6, overflow:"hidden" }}>
 <div style={{ width:`${(count/max)*100}%`, background:theme.coral, height:"100%", borderRadius:999 }} />
 </div>
 </div>
 ));
 })()}
 ...
 {[...kidGrowth].sort((a,b)=>a.date.localeCompare(b.date)).slice(-4).map((g,i)=>(
 <div key={i} style={{ flex:1, minWidth:70, background:`${theme.green}0a`, border:`1px solid ${theme.green}22`, borderRadius:6, padding:"10px 12px", textAlign:"center" }}>
 <div style={{ fontSize:9, color:theme.muted, textTransform:"uppercase", letterSpacing:"0.08em", fontFamily:"'Inter',sans-serif", marginBottom:4 }}>{g.date.slice(5)}</div>
 {g.height && <div style={{ fontSize:13, fontWeight:300, color:theme.green, fontFamily:"'Inter',sans-serif" }}>{g.height}cm</div>}
 {g.weight && <div style={{ fontSize:13, fontWeight:300, color:theme.accent, fontFamily:"'Inter',sans-serif" }}>{g.weight}kg</div>}
 </div>
 ))}
 </div>
 </div>
 ...
 {(() => {
 const naps = kidSleepLog.filter(s=>s.type==="Nap");
 const nights = kidSleepLog.filter(s=>s.type==="Nighttime Sleep");
 const avgNap = naps.length ? Math.round(naps.reduce((a,b)=>a+(parseInt(b.duration)||0),0)/naps.length) : 0;
 const avgNight = nights.length ? Math.round(nights.reduce((a,b)=>a+(parseInt(b.duration)||0),0)/nights.length) : 0;
 return (
 <div style={{ display:"flex", gap:8 }}>
 {avgNap>0 && <div style={{ flex:1, background:`${theme.purple}0a`, border:`1px solid ${theme.purple}22`, borderRadius:6, padding:"12px", textAlign:"center" }}>
 <div style={{ fontSize:20, fontWeight:300, color:theme.purple, fontFamily:"'Inter',sans-serif" }}>{avgNap}m</div>
 <div style={{ fontSize:9, color:theme.muted, textTransform:"uppercase", letterSpacing:"0.06em", fontFamily:"'Inter',sans-serif", marginTop:4 }}>Avg Nap</div>
 </div>}
 {avgNight>0 && <div style={{ flex:1, background:`${theme.blue}0a`, border:`1px solid ${theme.blue}22`, borderRadius:6, padding:"12px", textAlign:"center" }}>
 <div style={{ fontSize:20, fontWeight:300, color:theme.blue, fontFamily:"'Inter',sans-serif" }}>{Math.round(avgNight/60)}h {avgNight%60}m</div>
 <div style={{ fontSize:9, color:theme.muted, textTransform:"uppercase", letterSpacing:"0.06em", fontFamily:"'Inter',sans-serif", marginTop:4 }}>Avg Night</div>
 </div>}
 </div>
 );
 })()}
 ...
 {(() => {
 const total = kidVaccineList.length;
 const given = kidVaccineList.filter(v=>v.given).length;
 const pct = total ? Math.round((given/total)*100) : 0;
 return (
 <div>
 <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
 <div style={{ fontSize:13, fontFamily:"'Inter',sans-serif" }}>{given} of {total} vaccines complete</div>
 <div style={{ fontSize:13, fontWeight:300, color:theme.green, fontFamily:"'Inter',sans-serif" }}>{pct}%</div>
 </div>
 <div style={{ background:"#f0ebe4", borderRadius:999, height:8, overflow:"hidden" }}>
 <div style={{ width:`${pct}%`, background:theme.green, height:"100%", borderRadius:999, transition:"width 0.4s" }} />
 </div>
 </div>
 );
 })()}
 ...
 <div style={{ fontSize:10, color:theme.muted, textAlign:"center", fontFamily:"'Inter',sans-serif", lineHeight:1.7 }}>
 Trends grow richer as you log more data. Keep going!
 </div>
 </>}
 ...
 <div style={{ fontSize:12, color:theme.muted, lineHeight:1.7, fontFamily:"'Inter',sans-serif", marginBottom:8 }}>{report.desc}</div>
 <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
 {report.stats.map((s,si)=>(
 <span key={si} style={{ fontSize:10, color:report.color, background:`${report.color}10`, border:`1px solid ${report.color}28`, borderRadius:3, padding:"2px 8px", fontFamily:"'Inter',sans-serif", letterSpacing:"0.04em" }}>{s}</span>
 ))}
 </div>
 </div>
 <div style={{ padding:"8px 14px", borderRadius:4, background:`${report.color}12`, border:`1px solid ${report.color}33`, fontSize:11, fontWeight:500, letterSpacing:"0.06em", textTransform:"uppercase", fontFamily:"'Inter',sans-serif", color:report.color, whiteSpace:"nowrap", flexShrink:0, alignSelf:"center" }}>
 Export PDF
 </div>
 </div>
 </div>
 ...
 <div style={{ ...c.card, background:`${theme.yellow}0a`, border:`1px solid ${theme.yellow}28` }}>
 <div style={{ fontSize:10, color:theme.muted, letterSpacing:"0.1em", textTransform:"uppercase", fontFamily:"'Inter',sans-serif", marginBottom:8 }}>Coming in Version 2.0</div>
 <div style={{ fontSize:13, color:theme.text, lineHeight:1.8, fontFamily:"'Inter',sans-serif" }}>
 PDF export will be fully functional in the live app using {selectedKid.name}'s real logged data. Reports will be available to download, email, or share directly from the app.
 </div>
 </div>
 </>}
 ...
 <div style={{ display:"flex", gap:8 }}>
 <button style={{ ...c.btn(theme.teal) }} onClick={()=>setShowVisitScanner(true)}>Scan Summary</button>
 <button style={c.btn()} onClick={()=>openAdd("visits",F.visit,"visits",true)}>Add Visit</button>
 </div>
 </div>
 {showVisitScanner && <VisitScanner kid={selectedKid} onSave={(data)=>{ setVisits(prev=>[...prev,{id:Date.now(),kidId:selectedKid.id,...data}]); setShowVisitScanner(false); }} onClose={()=>setShowVisitScanner(false)} />}
 {kidVisits.length===0 && !showVisitScanner && <div style={{ color:theme.muted }}>No visits logged yet.</div>}
 ...
 <div><div style={{ fontSize:24, fontWeight:500, color:theme.green }}>{g.weight}<span style={{ fontSize:13 }}>kg</span></div><div style={{ fontSize:11, color:theme.muted }}>Weight</div></div>
 {g.headCirc&&<div><div style={{ fontSize:24, fontWeight:500, color:theme.purple }}>{g.headCirc}<span style={{ fontSize:13 }}>cm</span></div><div style={{ fontSize:11, color:theme.muted }}>Head Circ.</div></div>}
 </div>
 </EntryCard>
 ))}
 </>}
 ...
 <div>
 <div style={{ fontWeight:500, fontSize:16, color:group.color }}>{group.ageGroup}</div>
 <div style={{ fontSize:11, color:theme.muted }}>{groupGiven.length} of {group.items.length} recorded</div>
 </div>
 </div>
 ...
 <div style={{ fontWeight:500, fontSize:13, color: given ? theme.muted : theme.text, textDecoration: given ? "line-through" : "none", flex:1 }}>{item.name}</div>
 {item.isMed && <span style={c.badge(theme.orange)}>Medication</span>}
 {given && <span style={c.badge(theme.green)}> {given.date}</span>}
 </div>
 <div style={{ fontSize:11, color:theme.muted, marginTop:2, lineHeight:1.5 }}>{item.detail}</div>
 </div>
 </div>
 );
 })}
 ...
 <EntryCard openEdit={openEdit} setConfirmDelete={setConfirmDelete} c={c} key={v.id} item={v} dataKey="vaccines" label={v.name} fields={F.vaccine} kidScoped>
 <div style={{ display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:6 }}>
 <div style={{ fontWeight:500 }}> {v.name}</div>
 <div style={{ display:"flex", gap:6 }}>
 <div style={{ fontSize:13, color:theme.muted }}>{v.date}</div>
 <span style={c.badge(theme.green)}> Given</span>
 </div>
 </div>
 {v.notes && <div style={{ fontSize:13, color:theme.muted, marginTop:4 }}>{v.notes}</div>}
 </EntryCard>
 ))}
 </>}
 ...
 {kidVitalsList.map(v=><VitalsCard key={v.id} vital={v} dataKey="kidVitals" fields={F.kidVital} kidScoped />)}
 <div style={c.sTitle}>Lab Results</div>
 {kidLabsList.length===0&&<div style={{ color:theme.muted }}>No labs logged yet.</div>}
 ...
 <div>
 <div style={{ fontWeight:500, fontSize:16 }}> {age}</div>
 <div style={{ fontSize:12, color:theme.muted, marginTop:2 }}>{achieved.length} of {allItems.length} achieved</div>
 </div>
 <div style={{ display:"flex", alignItems:"center", gap:8 }}>
 <span style={c.badge(pct===100?theme.green:pct>50?theme.blue:theme.yellow)}>{pct===100?" Complete":`${pct}%`}</span>
 <span style={{ color:theme.muted, fontSize:14 }}>{isOpen?"":""}</span>
 </div>
 </div>
 ...
 <div style={{ fontSize:12, fontWeight:500, color:theme.accent, marginBottom:8, letterSpacing:"0.04em" }}>{category}</div>
 {items.map((item,i)=>{
 const done = kidMilestones.find(m=>m.age===age&&m.milestone===item);
 return (
 <div key={i} style={c.checkItem} onClick={()=>{
 if(!done) setMilestones([...milestones,{ id:Date.now(), kidId:selectedKid.id, age, milestone:item, category, achieved:true, date:today, notes:"" }]);
 else setMilestones(milestones.filter(m=>!(m.kidId===selectedKid.id&&m.age===age&&m.milestone===item)));
 }}>
 <div style={c.checkBox(!!done)}>{done?"":""}</div>
 <div style={{ flex:1 }}>
 <div style={{ fontSize:13, textDecoration:done?"line-through":"none", color:done?theme.muted:theme.text, lineHeight:1.5 }}>{item}</div>
 {done&&<div style={{ fontSize:11, color:theme.green, marginTop:2 }}> Achieved {done.date}</div>}
 </div>
 </div>
 );
 })}
 ...
 <div style={{ marginTop:8, fontSize:11, color:theme.muted, fontStyle:"italic" }}>
 Talk to your child's doctor if you have any concerns about their development.
 </div>
 </div>
 )}
 </div>
 ...
 <div style={{ fontSize:13, color:theme.muted, marginTop:2 }}>Every precious moment, captured forever.</div>
 </div>
 ...
 { title:"First Time Sleeping Through the Night", category:"Sleep" },
 { title:"First Solid Food", category:"Feeding" },
 { title:"First Word", category:"Communication" },
 { title:"First Roll Over", category:"Movement" },
 { title:"First Time Sitting Up",category:"Movement" },
 { title:"First Crawl", category:"Movement" },
 { title:"First Steps", category:"Movement" },
 { title:"First Tooth", category:"Health & Body" },
 { title:"First Haircut", category:"Health & Body" },
 { title:"First Bath", category:"Health & Body" },
 { title:"First Time at the Beach", category:"Experiences"},
 { title:"First Holiday", category:"Experiences" },
 { title:"First Time Meeting a Pet", category:"Experiences"},
 { title:"First Clap", category:"Social" },
 { title:"First Wave Bye-Bye", category:"Social" },
 { title:"First Peek-a-Boo", category:"Social" },
 { title:"First Taste of Ice Cream", category:"Feeding" },
 { title:"First Reaction to Music", category:"Personality"},
 ].filter(s => !kidFirstsList.find(f => f.title === s.title))
 .slice(0, 12)
 .map((s, i) => {
 const catColor = {
 "Social":"#b5836a","Movement":"#7a9e87","Feeding":"#c9a96e",
 "Communication":"#7a96a8","Sleep":"#8f8aa0","Health & Body":"#b5836a",
 "Experiences":"#6a9e96","Personality":"#c0614a",
 }[s.category] || theme.muted;
 return (
 <button key={i}
 onClick={()=>{ setForm({ title:s.title, category:s.category, date:today }); setModal({ type:"first", fields:F.first, dataKey:"kidFirsts", kidScoped:true, editing:false }); }}
 style={{ padding:"7px 14px", borderRadius:999, background:`${catColor}15`, color:catColor, border:`1px solid ${catColor}44`, cursor:"pointer", fontSize:12, fontWeight:500, transition:"all 0.2s" }}>
 {s.title}
 </button>
 );
 })}
 ...
 return (
 <div key={i} style={c.stat(catColor)}>
 <div style={{ ...c.statNum, color:catColor }}>{count}</div>
 <div style={c.statLabel}>{cat}</div>
 </div>
 );
 })}
 </div>
 ...
 <div style={{ color:theme.muted, fontSize:14, marginBottom:20 }}>Tap a suggestion above or "Add a First" to capture {selectedKid.name}'s precious moments.</div>
 </div>
 )}
 ...
 <button style={c.iconBtn(theme.coral)} onClick={()=>setConfirmDelete({ dataKey:"kidFirsts", id:f.id, label:f.title })}></button>
 </div>
 ...
 <div>
 <div style={{ fontWeight:500, fontSize:18, color:theme.text }}>{f.title}</div>
 <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginTop:4 }}>
 <span style={c.badge(catColor)}>{f.category}</span>
 {f.date && <span style={c.badge(theme.muted)}> {f.date}</span>}
 {f.ageAtTime && <span style={c.badge(theme.purple)}> {f.ageAtTime}</span>}
 </div>
 </div>
 </div>
 ...
 <div style={{ fontSize:12, fontWeight:500, fontFamily:"'Inter',sans-serif", marginBottom:2 }}>{item.title}</div>
 <div style={{ fontSize:12, color:theme.muted, lineHeight:1.6 }}>{item.desc}</div>
 </div>
 ))}
 ...
 <div style={{ marginTop:14, borderTop:`1px solid ${theme.border}`, paddingTop:14 }}>
 {[
 { title:"Log immediately", desc:"Log a feeding or diaper the moment it happens. Even a 5-minute delay can make you second-guess yourself." },
 { title:"Use the last log as your anchor", desc:"Before each feeding, glance at the last entry. Was it 2 hours ago? Left side last time? Your most recent entries are always at the top." },
 { title:"Share access with your partner", desc:"Two sets of eyes reduce errors. Show your partner how to log so nighttime handoffs are documented by whoever is awake." },
 { title:"Watch for patterns", desc:"After a few days of logging, patterns emerge. Patterns help you anticipate needs and spot anything unusual early." },
 { title:"Bring your log to well-visits", desc:"Pediatricians love data. Wet diapers per day, feeding frequency, and amounts all matter diagnostically." },
 { title:"Be kind to yourself", desc:"Forgetting is neurological, not negligence. This app exists precisely so that forgetting is okay. Log what you can, when you can." },
 ].map((item,i)=>(
 <div key={i} style={{ padding:"10px 12px", background:"#faf8f5", borderRadius:6, marginBottom:6 }}>
 <div style={{ fontSize:12, fontWeight:500, fontFamily:"'Inter',sans-serif", marginBottom:2 }}>{item.title}</div>
 <div style={{ fontSize:12, color:theme.muted, lineHeight:1.6 }}>{item.desc}</div>
 ...
 <div style={{ ...c.statNum, color:theme.yellow }}>{todayDiapers.filter(d=>d.type==="Wet"||d.type==="Both").length}</div>
 <div style={c.statLabel}>Wet Diapers</div>
 </div>
 <div style={c.stat(theme.green )}>
 <div style={{ ...c.statNum, color:theme.green }}>{todayDiapers.filter(d=>d.type==="Dirty"||d.type==="Both").length}</div>
 <div style={c.statLabel}>Dirty Diapers</div>
 </div>
 </div>
 );
 })()}
 ...
 <EntryCard openEdit={openEdit} setConfirmDelete={setConfirmDelete} c={c} key={f.id} item={f} dataKey="feedings" label={f.type} fields={F.feeding} kidScoped>
 <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
 <div style={{ fontWeight:500 }}> {f.type}</div>
 <span style={c.badge(theme.accent)}>{f.amount}</span>
 </div>
 <div style={{ fontSize:13, color:theme.muted }}>{f.date} at {f.time}</div>
 {f.notes&&<div style={{ fontSize:13, marginTop:6 }}>{f.notes}</div>}
 </EntryCard>
 ))}
 ...
 <EntryCard openEdit={openEdit} setConfirmDelete={setConfirmDelete} c={c} key={s.id} item={s} dataKey="kidSleep" label={`${s.type} ${s.date}`} fields={F.kidSleep} kidScoped>
 <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
 ...
 This record is for tracking purposes. Always share your child's allergy list with schools, daycares, coaches, and all healthcare providers. In an emergency, call <span style={{fontWeight:500}}>911</span> immediately.
 </p>
 </div>
 ...
 <div style={{ fontSize:13, marginTop:4 }}>Tap "Add Allergy" to log one</div>
 </div>
 )}
 ...
 <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
 <span style={c.badge(theme.blue)}>{a.category}</span>
 <span style={c.badge(sevColor)}>{a.severity}</span>
 </div>
 </div>
 {a.reaction && (
 <div style={{ fontSize:13, marginBottom:6 }}>
 <span style={{ fontWeight:500, color:theme.text }}>Reaction: </span>
 <span style={{ color:theme.muted }}>{a.reaction}</span>
 </div>
 )}
 {a.epiPen==="Yes" && (
 <div style={{ display:"inline-flex", alignItems:"center", gap:6, background:`${theme.coral}15`, border:`1px solid ${theme.coral}44`, borderRadius:999, padding:"4px 12px", marginBottom:8 }}>
 <span style={{ fontSize:14 }}></span>
 <span style={{ fontSize:12, fontWeight:500, color:theme.coral }}>EpiPen Prescribed</span>
 </div>
 )}
 {a.actionPlan && (
 <div style={{ background:`${theme.yellow}18`, border:`1px solid ${theme.yellow}44`, borderRadius:6, padding:"8px 12px", marginBottom:6 }}>
 <div style={{ fontSize:11, fontWeight:500, color:theme.yellow, marginBottom:3 }}> ACTION PLAN</div>
 ...
 <button style={c.navBtn(momView==="momDashboard")} onClick={()=>setMomView("momDashboard")}>Dashboard</button>
 <button style={c.navBtn(momView==="momTrends")} onClick={()=>setMomView("momTrends")}>Trends</button>
 </div>
 {/* Grouped collapsible sections */}
 {[
 { label:"Feeding & Pumping", tabs:[{id:"feeding",label:"Feeding"},{id:"pumping",label:"Pumping"}] },
 { label:"Wellness", tabs:[{id:"hydration",label:"Hydration"},{id:"nutrition",label:"Nutrition"},{id:"cycle",label:"Cycle"},{id:"momHealthTab",label:"My Health"}] },
 { label:"Health Records", tabs:[{id:"momVitalsTab",label:"Vitals & Labs"},{id:"momMeds",label:"My Meds"},{id:"momAllergies",label:"Allergies"},{id:"momVisits",label:"My Visits"}] },
 { label:"Resources", tabs:[{id:"postpartum",label:"Postpartum"},{id:"reminders",label:"Reminders"},{id:"momTips",label:"Mom Tips"},{id:"reports",label:"Reports"}] },
 ].map(group => {
 const isActive = group.tabs.some(t => t.id === momView);
 return (
 <details key={group.label} open={isActive} style={{ marginBottom:4 }}>
 <summary style={{ fontSize:10, fontWeight:500, letterSpacing:"0.1em", textTransform:"uppercase", fontFamily:"'Inter',sans-serif", color:isActive?theme.accent:theme.muted, cursor:"pointer", padding:"4px 0", listStyle:"none", display:"flex", alignItems:"center", justifyContent:"center", gap:6, textAlign:"center" }}>
 {group.label}
 ...
 <div style={{ display:"flex", gap:4, flexWrap:"wrap", paddingTop:6, justifyContent:"center" }}>
 {group.tabs.map(t => (
 <button key={t.id} style={c.navBtn(momView===t.id)} onClick={()=>setMomView(t.id)}>{t.label}</button>
 ))}
 </div>
 </details>
  );
 ...
 <div style={c.stat(theme.green )}><div style={{ ...c.statNum, color:theme.green }}>{momMeds.filter(m=>m.active).length}</div><div style={c.statLabel}>Active Meds</div></div>
 <div style={c.stat(theme.purple )}><div style={{ ...c.statNum, color:theme.purple }}>{events.filter(e=>e.memberId==="mom"&&e.date>=today).length}</div><div style={c.statLabel}>Upcoming Appts</div></div>
 <div style={c.stat(theme.teal )}><div style={{ ...c.statNum, color:theme.teal }}>{cycles.length}</div><div style={c.statLabel}>Cycles Logged</div></div>
 <div style={c.stat(theme.orange )}><div style={{ ...c.statNum, color:theme.orange }}>{nutritionLog.length}</div><div style={c.statLabel}>Nutrition Logs</div></div>
 </div>
 <div style={c.card}>
 <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
 ...
 <div key={ev.id} style={{ display:"flex", gap:12, padding:"10px 0", borderBottom:i<arr.length-1?`1px solid ${theme.border}`:"none", alignItems:"center" }}>
 <div style={{ width:40, flexShrink:0, textAlign:"center" }}>
 <div style={{ fontSize:18, fontWeight:300, fontFamily:"'Inter',sans-serif", color:MEMBER_COLORS[0], lineHeight:1 }}>{ev.date.slice(8)}</div>
 <div style={{ fontSize:9, color:theme.muted, textTransform:"uppercase", letterSpacing:"0.08em", fontFamily:"'Inter',sans-serif" }}>{["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][parseInt(ev.date.slice(5,7))-1]}</div>
 </div>
 ...
 <div style={{ display:"flex", gap:8 }}>{[1,2,4].map(n=><button key={n} style={c.smallBtn(theme.blue)} onClick={()=>quickAddWater(n)}>+{n} cup{n>1?"s":""}</button>)}</div>
 </div>
 </>}
 ...
 <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
 {[...momHealth].sort((a,b)=>a.date.localeCompare(b.date)).slice(-7).map((h,i)=>{
 const moodColors = { "Great":theme.green, "Good":theme.accent, "Okay":theme.yellow, "Low":theme.orange, "Struggling":theme.coral };
 ...
 {(() => {
 const logs = momHealth.filter(h=>h.sleep).slice(-7);
 const avg = (logs.reduce((a,b)=>a+(parseFloat(b.sleep)||0),0)/logs.length).toFixed(1);
 const max = Math.max(...logs.map(l=>parseFloat(l.sleep)||0));
 return (
 <div>
 <div style={{ fontSize:28, fontWeight:300, color:theme.purple, fontFamily:"'Inter',sans-serif", marginBottom:12 }}>{avg}<span style={{ fontSize:13, color:theme.muted }}> hrs avg</span></div>
 <div style={{ display:"flex", gap:4, alignItems:"flex-end", height:50 }}>
 {logs.map((h,i)=>{
 const hrs = parseFloat(h.sleep)||0;
 return (
 <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
 <div style={{ width:"100%", background:theme.purple, borderRadius:"2px 2px 0 0", height:`${max>0?(hrs/max)*40:0}px`, minHeight:2 }} />
 <div style={{ fontSize:8, color:theme.muted, fontFamily:"'Inter',sans-serif" }}>{h.date?.slice(8)}</div>
 </div>
 );
 })}
 </div>
 ...
 const cups = waterLog.filter(w=>w.date===ds).reduce((a,b)=>a+(b.cups||0),0);
 return { date:ds.slice(5), cups };
 }).reverse();
 const avg = (last7.reduce((a,b)=>a+b.cups,0)/7).toFixed(1);
 return (
 <div>
 <div style={{ fontSize:28, fontWeight:300, color:theme.blue, fontFamily:"'Inter',sans-serif", marginBottom:12 }}>{avg}<span style={{ fontSize:13, color:theme.muted }}> cups avg</span></div>
 <div style={{ display:"flex", gap:4, alignItems:"flex-end", height:50 }}>
 {last7.map((d,i)=>(
 <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
 <div style={{ width:"100%", background:d.cups>=13?theme.green:theme.blue, borderRadius:"2px 2px 0 0", height:`${Math.min((d.cups/13)*40,40)}px`, minHeight:2 }} />
 <div style={{ fontSize:8, color:theme.muted, fontFamily:"'Inter',sans-serif" }}>{d.date}</div>
 </div>
 ))}
 </div>
 </div>
 ...
 {(() => {
 const types = {};
 feedingSessions.forEach(f=>{ types[f.type]=(types[f.type]||0)+1; });
 const total = feedingSessions.length;
 return Object.entries(types).map(([type,count])=>(
 <div key={type} style={{ marginBottom:10 }}>
 <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
 <div style={{ fontSize:12, fontFamily:"'Inter',sans-serif" }}>{type}</div>
 <div style={{ fontSize:12, color:theme.muted }}>{Math.round((count/total)*100)}%</div>
 </div>
 <div style={{ background:"#f0ebe4", borderRadius:999, height:5, overflow:"hidden" }}>
 <div style={{ width:`${(count/total)*100}%`, background:theme.accent, height:"100%", borderRadius:999 }} />
 </div>
 </div>
 ));
 })()}
 ...
 <div style={{ fontSize:12, color:theme.muted, lineHeight:1.7, fontFamily:"'Inter',sans-serif" }}>{item.desc}</div>
 </div>
 <div onClick={()=>{}} style={{ width:44, height:24, borderRadius:999, background:item.toggle?item.color:"#e8e2db", cursor:"pointer", position:"relative", flexShrink:0, transition:"background 0.2s" }}>
 <div style={{ width:18, height:18, borderRadius:"50%", background:"#fff", position:"absolute", top:3, left:item.toggle?23:3, transition:"left 0.2s", boxShadow:"0 1px 4px rgba(0,0,0,0.15)" }} />
 </div>
 </div>
 ))}
 ...
 <div style={{ fontSize:10, color:theme.muted, letterSpacing:"0.1em", textTransform:"uppercase", fontFamily:"'Inter',sans-serif", marginBottom:8 }}>Coming in Version 2.0</div>
 <div style={{ fontSize:13, color:theme.text, lineHeight:1.8, fontFamily:"'Inter',sans-serif" }}>
 Push notifications, custom reminder times, snooze options, and caregiver notification sharing will be fully functional in the live app. Your developer will implement Firebase Cloud Messaging (FCM) for delivery.
 </div>
 </div>
 </>}
 ...
 <div style={{ fontSize:10, color:theme.muted, letterSpacing:"0.1em", textTransform:"uppercase", fontFamily:"'Inter',sans-serif", marginBottom:8 }}>Coming in Version 2.0</div>
 <div style={{ fontSize:13, color:theme.text, lineHeight:1.8, fontFamily:"'Inter',sans-serif" }}>
 PDF generation will be fully functional in the live app using your real logged data. Your developer will implement server-side PDF rendering. Reports will be available to download, share, or email directly from the app.
 </div>
 </div>
 </>}
 ...
 <div style={c.statLabel}>Breastfeed</div>
 </div>
 <div style={c.stat(theme.blue)}>
 <div style={{ ...c.statNum, color:theme.blue }}>{feedingSessions.filter(f=>f.type==="Formula Bottle"||f.type==="Breast Milk Bottle"||f.type==="Combination").length}</div>
 <div style={c.statLabel}>Bottle Sessions</div>
 </div>
 <div style={c.stat(theme.green)}>
 <div style={{ ...c.statNum, color:theme.green }}>
 {feedingSessions.reduce((sum,f)=>sum+(parseFloat(f.oz)||0),0).toFixed(1)}
 </div>
 <div style={c.statLabel}>Total oz Logged</div>
 </div>
 <div style={c.stat(theme.purple)}>
 <div style={{ ...c.statNum, color:theme.purple }}>{feedingSessions.length}</div>
 <div style={c.statLabel}>Total Sessions</div>
 </div>
 </div>
 ...
 const isFormula = f.type==="Formula Bottle";
 const isPump = f.type==="Pump";
 const isBreast = f.type==="Breastfeed";
 const icon = isFormula?"": isPump?"": isBreast?"":"";
 const color = isFormula?theme.blue: isPump?theme.purple: isBreast?theme.accent: theme.green;
 return (
 <EntryCard openEdit={openEdit} setConfirmDelete={setConfirmDelete} c={c} key={f.id} item={f} dataKey="feedingSessions" label={f.type} fields={F.momFeeding}>
 <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6 }}>
 <div style={{ fontWeight:500, fontSize:16 }}>{icon} {f.type}</div>
 <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
 {f.oz && <span style={c.badge(color)}>{f.oz} oz</span>}
 {f.side && f.side!=="N/A" && <span style={c.badge(color)}>{f.side}</span>}
 </div>
 </div>
 ...
 <span style={c.badge(theme.blue)}> {f.formulaBrand}</span>
 </div>
 )}
 {f.notes && <div style={{ fontSize:13, color:theme.muted, marginTop:4 }}>{f.notes}</div>}
 </EntryCard>
 );
 })}
 </>}
 ...
 <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
 {[
 { label:"Room Temp", val:"Up to 4 hrs", color:theme.yellow },
 { label:"Fridge", val:"Up to 4 days", color:theme.blue },
 { label:"Freezer", val:"Up to 12 months",color:theme.purple },
 { label:"Deep Freeze",val:"Up to 12 months",color:theme.teal },
 ].map((g,i)=>(
 <div key={i} style={{ background:`${g.color}15`, border:`1px solid ${g.color}44`, borderRadius:6, padding:"8px 14px" }}>
 <div style={{ fontSize:11, color:theme.muted, fontWeight:500 }}>{g.label}</div>
 <div style={{ fontSize:14, fontWeight:500, color:g.color }}>{g.val}</div>
 </div>
 ))}
 </div>
 ...
 <div style={c.statLabel}> In Fridge</div>
 </div>
 <div style={c.stat(theme.purple)}>
 <div style={{ ...c.statNum, color:theme.purple }}>{freezerOz.toFixed(1)}<span style={{ fontSize:13 }}>oz</span></div>
 <div style={c.statLabel}> In Freezer</div>
 </div>
 <div style={c.stat(theme.green)}>
 <div style={{ ...c.statNum, color:theme.green }}>{totalAvail.toFixed(1)}<span style={{ fontSize:13 }}>oz</span></div>
 <div style={c.statLabel}>Total Available</div>
 </div>
 <div style={c.stat(theme.accent)}>
 <div style={{ ...c.statNum, color:theme.accent }}>{todayPumped.toFixed(1)}<span style={{ fontSize:13 }}>oz</span></div>
 <div style={c.statLabel}>Pumped Today</div>
 </div>
 </div>
 ...
 <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
 <h3 style={{ margin:0, fontWeight:500, fontSize:18 }}>Use Milk from Inventory</h3>
 ...
 <div style={{ fontSize:24, fontWeight:500, color:theme.green }}>{totalAvail.toFixed(1)} oz</div>
 </div>
 <div style={{ marginBottom:14 }}>
 <label style={c.label}>How many oz to use?</label>
 ...
 <div style={{ color:theme.coral, fontSize:13, marginBottom:10 }}> Not enough milk available ({totalAvail.toFixed(1)} oz total)</div>
 )}
 <div style={{ display:"flex", gap:10 }}>
 <button style={c.btn(theme.coral)} onClick={handleUse} disabled={!useOz || parseFloat(useOz)>totalAvail}>Deduct {useOz||"0"} oz</button>
 <button style={c.ghostBtn} onClick={()=>setShowUseModal(false)}>Cancel</button>
 </div>
 </div>
 </div>
 )}
 ...
 const expColor = daysLeft<=1?theme.coral:daysLeft<=3?theme.yellow:theme.green;
 return (
 <EntryCard openEdit={openEdit} setConfirmDelete={setConfirmDelete} c={c} key={item.id} item={item} dataKey="milkInventory" label={item.label} fields={F.milkEntry}>
 <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
 <div style={{ fontWeight:500, fontSize:16 }}>
 {item.stored==="Fridge"?"":""} {item.label}
 </div>
 <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
 <span style={c.badge(item.stored==="Fridge"?theme.blue:theme.purple)}>{item.stored}</span>
 <span style={c.badge(theme.accent)}>{item.oz} oz</span>
 </div>
 </div>
 <div style={{ fontSize:13, color:theme.muted }}>Pumped: {item.pumped}</div>
 <div style={{ fontSize:13, marginTop:4 }}>
 <span style={{ color:expColor, fontWeight:500 }}>
 {daysLeft<=0 ? " Expired" : daysLeft===1 ? " Use today" : `Use by: ${item.expires} (${daysLeft} days)`}
 </span>
 </div>
 </EntryCard>
 );
 ...
 <div style={{ fontSize:13 }}> Left: <span style={{fontWeight:500}}>{s.leftOz} oz</span></div>
 <div style={{ fontSize:13 }}> Right: <span style={{fontWeight:500}}>{s.rightOz} oz</span></div>
 <div style={{ fontSize:13, color:theme.muted }}> {s.duration} min</div>
 </div>
 <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
 <span style={c.badge(s.stored==="Fridge"?theme.blue:s.stored==="Freezer"?theme.purple:theme.green)}>{s.stored}</span>
 {s.bagLabel && <span style={c.badge(theme.muted)}> {s.bagLabel}</span>}
 </div>
 {s.notes && <div style={{ fontSize:13, color:theme.muted, marginTop:6 }}>{s.notes}</div>}
 </EntryCard>
 ))}
 </>}
 ...
 <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>{[1,2,4,8].map(n=><button key={n} style={c.smallBtn(theme.blue)} onClick={()=>quickAddWater(n)}>+{n} cup{n>1?"s":""}</button>)}</div>
 {waterPct>=100&&<div style={{ marginTop:12, fontWeight:500, color:theme.green }}> Goal reached! Great job, mama!</div>}
 </div>
 <div style={c.card}>
 ...
 <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
 {n.protein==="Yes"&&<span style={c.badge(theme.green)}> Protein</span>}
 {n.iron==="Yes" &&<span style={c.badge(theme.coral)}> Iron</span>}
 {n.calcium==="Yes"&&<span style={c.badge(theme.blue)}> Calcium</span>}
 </div>
 </EntryCard>
 ))}
 </>}
 ...
 {momVitals.map(v=><VitalsCard key={v.id} vital={v} dataKey="momVitals" fields={F.momVital} />)}
 <div style={c.sTitle}>Lab Results</div>
 {momLabs.length===0&&<div style={{ color:theme.muted }}>No labs logged yet.</div>}
 ...
 <div style={{ display:"flex", gap:8 }}>
 <button style={{ ...c.btn(theme.teal) }} onClick={()=>setShowMamaScanner(true)}>Scan Summary</button>
 ...
 onSave={(data)=>{ setMomVisits(prev=>[...prev,{id:Date.now(),...data}]); setShowMamaScanner(false); }}
 onClose={()=>setShowMamaScanner(false)}
 />
 )}
 {momVisits.length===0 && !showMamaScanner && <div style={{ color:theme.muted, fontSize:13 }}>No visits logged yet.</div>}
 ...
 <div><div style={{ fontSize:22, fontWeight:500, color:theme.coral }}>{h.painLevel}/10</div><div style={{ fontSize:11, color:theme.muted }}>Pain</div></div>
 </div>
 {h.notes&&<div style={{ fontSize:14, marginTop:10 }}>{h.notes}</div>}
 </EntryCard>
 ))}
 </>}
 ...
 <div key={i} style={{ padding:"8px 12px", background:"rgba(143,138,160,0.08)", borderRadius:6, marginBottom:6 }}>
 <div style={{ fontSize:12, fontWeight:500, fontFamily:"'Inter',sans-serif" }}>{item.t}</div>
 <div style={{ fontSize:12, color:"#8a8480", lineHeight:1.6, marginTop:2 }}>{item.d}</div>
 </div>
 ))}
 ...
 {ppSection==="checklist" && (
 <div style={{ marginTop:14, borderTop:"1px solid #e8e2db", paddingTop:14 }}>
 {postpartumChecklist.map(sec=>{
 const doneCount=sec.items.filter(item=>checklistDone[sec.category+"||"+item]).length;
 return (
 <div key={sec.category} style={{ marginBottom:16 }}>
 <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
 <div style={{ fontSize:10, fontWeight:500, letterSpacing:"0.08em", textTransform:"uppercase", fontFamily:"'Inter',sans-serif", color:"#8a8480" }}>{sec.category}</div>
 <div style={{ fontSize:10, color:doneCount===sec.items.length?"#7a9e87":"#b5836a", fontFamily:"'Inter',sans-serif" }}>{doneCount}/{sec.items.length}</div>
 </div>
 <div style={{ background:"#e8e2db", borderRadius:999, height:4, overflow:"hidden", marginBottom:8 }}>
 <div style={{ width:(sec.items.length?Math.round((doneCount/sec.items.length)*100):0)+"%", background:"#7a9e87", height:"100%", borderRadius:999, transition:"width 0.4s" }} />
 </div>
 {sec.items.map(item=>{
 const done=checklistDone[sec.category+"||"+item];
 return (
 ...
 {a.reaction && (
 <div style={{ fontSize:13, marginBottom:6 }}>
 <span style={{ fontWeight:500 }}>Reaction: </span>
 <span style={{ color:theme.muted }}>{a.reaction}</span>
 </div>
 )}
 {a.epiPen==="Yes" && (
 ...
 <span style={{ fontSize:9, color:group.color, background:`${group.color}12`, border:`1px solid ${group.color}28`, borderRadius:3, padding:"2px 8px", fontFamily:"'Inter',sans-serif", letterSpacing:"0.06em", textTransform:"uppercase" }}>{group.tag}</span>
 </div>
 ...
 <InfoBanner gradient="#faf8f5">You take care of everyone. These are here to take care of you.</InfoBanner>
 {momHealthTips.map((t,i) => (
 <div key={i} style={{ ...c.card, borderLeft:`3px solid ${theme.teal}` }}>
 <p style={{ margin:0, fontSize:13, lineHeight:1.9, color:theme.text, fontFamily:"'Inter',sans-serif", fontWeight:400 }}>{t.tip}</p>
 </div>
 ))}
 </>}
 ...
 <span style={{ fontSize:32 }}></span>
 <div>
 <div style={{ fontWeight:500, fontSize:16, color:theme.accent }}>Coming in Version 2.0</div>
 <div style={{ fontSize:13, color:theme.muted }}>Shared access is currently in development. Here's a preview of what's coming.</div>
 </div>
 </div>
 </div>
 ...
 <div style={{ width:32, height:32, borderRadius:"50%", background:`linear-gradient(135deg,${theme.accent},${theme.purple})`, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:500, fontSize:14 }}>{item.step}</div>
 <div>
 <div style={{ fontWeight:500, fontSize:14 }}>{item.icon} {item.title}</div>
 <div style={{ fontSize:13, color:theme.muted, marginTop:3, lineHeight:1.6 }}>{item.desc}</div>
 </div>
 </div>
 ))}
 </div>
 ...
 {[
 { role:" Admin (You)", color:theme.accent, view:true, log:true, edit:true, del:true, invite:true, desc:"Full access to everything. Only you can invite/remove members." },
 { role:" Co-Parent", color:theme.purple, view:true, log:true, edit:true, del:true, invite:false, desc:"Full logging and editing access. Cannot manage family members." },
 { role:" Caregiver", color:theme.blue, view:true, log:true, edit:false, del:false, invite:false, desc:"Can log feedings, diapers, and symptoms. Cannot edit or delete." },
 { role:" View Only", color:theme.green, view:true, log:false, edit:false, del:false, invite:false, desc:"Can see all data but cannot add or change anything." },
 ].map((r,i)=>(
 <div key={i} style={{ marginBottom:14, padding:14, background:`${r.color}0d`, border:`1px solid ${r.color}33`, borderRadius:6 }}>
 <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8, flexWrap:"wrap", gap:6 }}>
 <div style={{ fontWeight:500, fontSize:14, color:r.color }}>{r.role}</div>
 <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
 {[["View",r.view],["Log",r.log],["Edit",r.edit],["Delete",r.del],["Invite",r.invite]].map(([label,allowed])=>(
 <span key={label} style={{ fontSize:11, padding:"3px 10px", borderRadius:999, background: allowed ? `${theme.green}20` : `${theme.muted}15`, color: allowed ? theme.green : theme.muted, border:`1px solid ${allowed?theme.green:theme.border}`, fontWeight:500 }}>
 {allowed ? "" : ""} {label}
 </span>
 ))}
 </div>
 </div>
 <div style={{ fontSize:12, color:theme.muted, lineHeight:1.6 }}>{r.desc}</div>
 </div>
 ))}
 </div>
 ...
 { name:"Grandma Rose", email:"rose@email.com", role:" View Only", color:theme.green, status:"Active", avatar:"" },
 { name:"Nanny Invite", email:"nanny@email.com", role:" Caregiver", color:theme.blue, status:"Pending", avatar:"" },
 ].map((m,i)=>(
 <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 0", borderBottom: i<3 ? `1px solid ${theme.border}` : "none" }}>
 <div style={{ width:40, height:40, borderRadius:"50%", background:`${m.color}20`, border:`2px solid ${m.color}44`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0 }}>{m.avatar}</div>
 <div style={{ flex:1 }}>
 <div style={{ fontWeight:400, fontSize:18, fontFamily:"'Cormorant Garamond',serif", fontStyle:"italic" }}>{m.name}</div>
 <div style={{ fontSize:12, color:theme.muted }}>{m.email}</div>
 </div>
 <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:4 }}>
 <span style={c.badge(m.color)}>{m.role}</span>
 <span style={{ ...c.badge(m.status==="Active"?theme.green:theme.yellow), fontSize:10 }}>{m.status}</span>
 </div>
 </div>
 ))}
 {/* Mock invite button */}
 <button style={{ ...c.btn(), width:"100%", marginTop:16, opacity:0.6, cursor:"not-allowed" }}>
 + Invite a Caregiver (Available in v2.0)
 </button>
 </div>
 ...
 <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
 {[" Feeding logs"," Diaper logs"," Sleep tracking","Medications"," Symptoms"," Family calendar"," Vaccine records"," Growth data"," Allergy alerts"," Doctor visits"," Vitals & labs"," Milestones"].map((item,i)=>(
 <span key={i} style={{ ...c.badge(theme.green), fontSize:12, padding:"6px 12px" }}>{item}</span>
 ))}
 </div>
 </div>
 ...
 <div><div style={{ fontWeight:500, fontSize:13 }}>{item.t}</div><div style={{ fontSize:12, color:theme.muted, lineHeight:1.5, marginTop:2 }}>{item.d}</div></div>
 </div>
 ))}
 </div>
 </div>
 ...
 <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
 {[
 " Zero ads on all plans", " No data sold to advertisers",
 " No sponsored content", " No tracking pixels",
 " No pop-ups or banners", " Revenue from users only",
 ].map((b,i)=>(
 <span key={i} style={{ fontSize:12, padding:"5px 12px", borderRadius:999, background:`${theme.green}18`, color:theme.green, border:`1px solid ${theme.green}44`, fontWeight:500 }}>{b}</span>
 ))}
 </div>
 </div>
 ...
 <div style={{ fontSize:22, fontWeight:500, color:theme.green }}>$0</div>
 </div>
 <div style={{ fontSize:12, color:theme.muted, marginBottom:10 }}>Free forever. No credit card needed.</div>
 <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
 {[
 " 1 child profile",
 " Symptom, med & visit logging",
 " Vaccine schedule (AAP 2026)",
 ...
 <div key={i} style={{ fontSize:13, color: f.startsWith("") ? theme.muted : theme.text, display:"flex", gap:8 }}>
 <span style={{ color: f.startsWith("") ? theme.green : theme.muted, fontWeight:500, flexShrink:0 }}>{f.slice(0,1)}</span>
 <span>{f.slice(2)}</span>
 </div>
 ))}
 </div>
 ...
 <div style={{ fontWeight:500, fontSize:16, color:theme.accent }}> Premium Monthly</div>
 <div>
 <div style={{ fontSize:22, fontWeight:500, color:theme.accent }}>$6.99<span style={{ fontSize:13, fontWeight:500, color:theme.muted }}>/mo</span></div>
 </div>
 </div>
 <div style={{ fontSize:12, color:theme.muted, marginBottom:10 }}>Cancel anytime. Billed monthly.</div>
 <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
 {[
 " Everything in Free",
 " Unlimited children",
 " Shared family access (up to 5)",
 " AI Sleep Planner",
 ...
 <div key={i} style={{ fontSize:13, color:theme.text, display:"flex", gap:8 }}>
 <span style={{ color:theme.accent, fontWeight:500, flexShrink:0 }}></span>
 <span>{f.slice(2)}</span>
 </div>
 ))}
 </div>
 ...
 <div key={i} style={{ fontSize:13, color:theme.text, display:"flex", gap:8 }}>
 <span style={{ color:theme.yellow, fontWeight:500, flexShrink:0 }}></span>
 <span>{f.slice(2)}</span>
 </div>
 ))}
 </div>
 <div style={{ marginTop:12, padding:"8px 12px", background:`${theme.yellow}18`, borderRadius:6, fontSize:12, color:theme.text }}>
 Break-even vs monthly: just 8 months. After that it's pure savings.
 </div>
 </div>
 ...
 {/* Header */}
 <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1fr 1fr", background:`${theme.accent}15`, padding:"10px 14px", borderBottom:`1px solid ${theme.border}` }}>
 {["Feature","Free","Monthly","Lifetime"].map((h,i)=>(
 <div key={i} style={{ fontWeight:500, fontSize:12, color: i===0?theme.muted:i===1?theme.green:i===2?theme.accent:theme.yellow, textAlign: i>0?"center":"left" }}>{h}</div>
 ))}
 </div>
 {[
 ["Symptom & med logging", "","",""],
 ...
 ["CDC milestone checklist", "","",""],
 ["Feeding & diaper log", "","",""],
 ["Postpartum care & PPD resources", "","",""],
 ["Nurse tips", "","",""],
 ["Number of children", "1","Unlimited","Unlimited"],
 ["Shared family access", "","Up to 5","Unlimited"],
 ["AI Sleep Planner", "","",""],
 ["Medication reminders", "","",""],
 ["Vitals & labs", "","",""],
 ["Allergy action plans", "","",""],
 ["Data export (PDF)", "","",""],
 ["Future updates", "",""," Always"],
 ["Ads", "None","None","None"],
 ].map((row, i)=>(
 <div key={i} style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1fr 1fr", padding:"9px 14px", borderBottom: i<14?`1px solid ${theme.border}`:"none", background: i%2===0?"#fff":"#ffffff" }}>
 <div style={{ fontSize:13, color:theme.text }}>{row[0]}</div>
 {[row[1],row[2],row[3]].map((v,j)=>(
 <div key={j} style={{ textAlign:"center", fontSize:13, fontWeight: v===""||v==="None"?700:500, color: v===""?theme.green: v===""?theme.muted: v==="None"?theme.green: j===1?theme.accent:theme.yellow }}>
 {v}
 </div>
 ))}
 </div>
 ...
 {modal?.isEvent ? (
 <div style={c.overlay} onClick={()=>setModal(null)}>
 <div style={c.modalBox} onClick={e=>e.stopPropagation()}>
 <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
 <h3 style={{ margin:0, fontSize:18, fontWeight:500 }}>{modal.editing?" Edit Event":" Add Event"}</h3>
 <button onClick={()=>setModal(null)} style={{ background:"none", border:"none", color:theme.muted, cursor:"pointer", fontSize:20 }}></button>
 </div>
 {modal.fields.map(f=><FormField key={f.field} {...f} form={form} setForm={setForm} />)}
 <div style={{ display:"flex", gap:10, marginTop:20 }}>
 <button style={c.btn()} onClick={saveEvent}>{modal.editing?"Save Changes":"Add Event"}</button>
 <button style={c.ghostBtn} onClick={()=>setModal(null)}>Cancel</button>
 </div>
 </div>
 </div>
 ) : <EntryModal modal={modal} form={form} setForm={setForm} setModal={setModal} saveEntry={saveEntry} />}
 ...
 {[["Name","name","text"],["Date of Birth","dob","date"],["Avatar Emoji","avatar","text"]].map(([label,field,type])=>(
 <div key={field} style={{ marginBottom:14 }}>
 <label style={c.label}>{label}</label>
 <input style={c.input} type={type} value={newKid[field]} onChange={e=>{ const v=e.target.value; setNewKid(prev=>({...prev,[field]:v})); }} />
 </div>
 ))}
 <div style={{ marginBottom:14 }}>
 <label style={c.label}>Blood Type</label>
 <select style={c.input} value={newKid.bloodType} onChange={e=>{ const v=e.target.value; setNewKid(prev=>({...prev,bloodType:v})); }}>
 {["A+","A-","B+","B-","AB+","AB-","O+","O-"].map(t=><option key={t}>{t}</option>)}
 </select>
 </div>
 <div style={{ display:"flex", gap:10 }}>
 <button style={c.btn()} onClick={addKid}>Add Child</button>
 <button style={c.ghostBtn} onClick={()=>setShowAddKid(false)}>Cancel</button>
 </div>
 </div>
 </div>
 )}
 ...
 { title:"Access your data without permission", desc:"Our team does not access individual user records except when explicitly required to resolve a support issue you have reported, with your consent." },
 ].map((item,i)=>(
 ...
 { title:"Minimal data collection", desc:"We only collect what is necessary for the app to function. We do not collect location data, contacts, camera roll, or microphone access beyond what you explicitly use." },
 ].map((item,i)=>(
 <div key={i} style={{ display:"flex", gap:12, padding:"12px 0", borderBottom:i<4?`1px solid ${theme.border}`:"none" }}>
 ...
 <p style={{ margin:0, fontSize:13, lineHeight:1.9, color:theme.text, fontFamily:"'Inter',sans-serif" }}>
 The AI Sleep Planner and Visit Scanner use the Anthropic Claude API. When you use these features, anonymized data is sent securely to Anthropic's API to generate your results. This data is not stored by Anthropic beyond the immediate request and is never used to train AI models. We never send identifiable personal health information to any AI service without your explicit consent.
 </p>
 </div>
 ...
 {["Privacy Policy","Terms of Service","HIPAA Notice","Contact Us"].map((link,i)=>(
 <div key={i} style={{ fontSize:11, color:theme.accent, letterSpacing:"0.04em", fontFamily:"'Inter',sans-serif", cursor:"pointer", borderBottom:`1px solid ${theme.accent}44`, paddingBottom:1 }}>{link}</div>
 ))}
 </div>
 </div>
 ...
 <div style={{ width:32, height:1, background:"#e8e2db", margin:"0 auto 10px" }} />
